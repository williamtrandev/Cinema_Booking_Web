...
<body>
    ...
    <section class="my-section">
        <div class="container mb-5">
            ...
            if (!isset($_SESSION['id-booking']) || !isset($data_ghe)) {
                $data_ghe = '';
            }
            if (!isset($info)) {
                $info = '';
            }
            $this->render($content, ['data' => $data_pass, 'suatchieu' => $suatchieu, 'ghe' => $data_ghe, 'thongtindat' => $info]);
            if (!empty($banner)) {
                $this->render($banner, ['info'=>$info_banner]);
            }
            if (!empty($showing))
                $this->render($showing, ['cards' => $cards]);
            if (!empty($coming))
                $this->render($coming, ['imgs' => $imgs, 'detail' => $detail]);
            ?>
        </div>
